	...
	if Patient then
		if oldMan then
			if Player.Functions.RemoveMoney("cash", 5000, "revived-player") then
				...
				TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['medikit'], "remove")
...
RegisterNetEvent('hospital:server:Usemedikit', function(targetId)
	local src = source
...
RegisterNetEvent('hospital:server:removeMedikit', function()
	local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Functions.CreateUseableItem("medikit", function(source, item)
	...
	local Player = QBCore.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
		TriggerClientEvent("hospital:client:UseMedikit", src)
	end
end)
